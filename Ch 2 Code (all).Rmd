...
avgTNTPbloom<-merge(noTNTPbloom, avgTNTP, by=c("LakeID", "Sample_Year"), all.x=TRUE, all.y=FALSE)
...
hadTNTP<-Bloom[!is.na(Bloom$TN_TP),]
completeBloom<-rbind(hadTP, avgTPbloom)
completeBloom<-rbind(completeBloom, hadTNTP)
...
```{r}
#create linear models 
TNTP_invaded.lm<-lm(log(invadedBlooms$ESF_Microcystin+.1)~log(invadedBlooms$TN_TP+.1))
TNTP_uninvaded.lm<-lm(log(uninvadedBlooms$ESF_Microcystin+.1)~log(uninvadedBlooms$TN_TP+.1))

summary(TNTP_invaded.lm)
summary(TNTP_uninvaded.lm)

TNTP_invaded.lm.sum<-summary(TNTP_invaded.lm)
TNTP_uninvaded.lm.sum<-summary(TNTP_uninvaded.lm)
```

```{r}
ggplot(completeBloom, aes(x=TN_TP, y=ESF_Microcystin, color=Dreissenids, shape=Dreissenids)) +
  geom_point(cex=2) + 
  geom_smooth(data=subset(completeBloom, Dreissenids == "Invaded" ),
...
ggsave("./output_figures/Fig 2-3 Global.png")
```
