...
###Fit the model
**Update 06/05** `Model failed to converge` removed `Sample_Year` and `Sample_Month` for having very small variance
```{r}
TNTP<-lmer(log(TN_TP) ~ Dreissenids + log(CA.SA) + Mean_Depth_m  + (1|LakeID) + (1|Sample_Year:LakeID), data=TN_TP)
...
-**Updated 06/02** Trying Gamma distribution to fix equal variance issue

###Fit the model
```{r}
BSTP<-glmer(TP_mg.L+1 ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m)  + (1|Sample_Month) + (1|LakeID) + (1|Sample_Year:LakeID), data=BSCSLAP, family=Gamma(link="log"))
...
###Fit the model
**Update 06/05** trying gamma distribution 
-model failed to converge, removed Year and Month (small variance)
```{r}
OWTC<-glmer(True_Color_PTU ~ Dreissenids + CA.SA + Mean_Depth_m  + (1|LakeID) + (1|Sample_Year:LakeID), data=OWCSLAP, family=Gamma(link="log"))
...
SBCSLAP<-SBCSLAP[,1:44]
SBCSLAP<-merge(SBCSLAP, avgTNTP, by=c("LakeID", "Sample_Year"), all.x=TRUE, all.y=FALSE)
```

###Fit the model
```{r}
SBmicro<-lmer(log(ESF_Microcystin_ug.L) ~ Dreissenids + TN_TP + log(CA.SA) + Mean_Depth_m + (1|Sample_Year) + (1|LakeID), data=SBCSLAP)
...
```{r}
SBCSLAP$binary_Micro <- ifelse(SBCSLAP$ESF_Microcystin_ug.L >= 10, "1", "0")
SBCSLAP$binary_Micro <- as.factor(SBCSLAP$binary_Micro)
```

###Fit the binary model 
```{r}
SBmicro_bin<-lmer(binary_Micro ~ Dreissenids + TN_TP + log(CA.SA) + Mean_Depth_m + (1|Sample_Year) + (1|LakeID), data=SBCSLAP)
