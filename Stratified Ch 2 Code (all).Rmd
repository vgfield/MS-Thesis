---
title: "Stratified Ch 2 Code (All)"
...
```{r, stratified}
#which lakes don't have bottom samples? We want to remove those

Strat <- CSLAP[CSLAP$Info_Type == "BS", ]
Strat$LakeID <- droplevels(Strat$LakeID) #54 lakes have BS samples, meaning 14 don't
StratLake<-as.data.frame(levels(Strat$LakeID))
colnames(StratLake)[1]<- "LakeID"

#which 14 lakes are not stratified? 
noStrat<- as.character(levels(CSLAP$LakeID)[!(levels(CSLAP$LakeID) %in% levels(Strat$LakeID))])

#remove these lakes from CSLAP df 
CSLAP<-merge(CSLAP, StratLake)
CSLAP$LakeID <- droplevels(CSLAP$LakeID)
```

Now we have a clean, full dataset of stratified lakes. Next, we need to create the reduced dataset using propensity score matching
...
Lake_List<-merge(Lake_List, StratLake) #remove non-stratified lakes
```
...
write.csv(VariableSummaries, "./output_data/StratifiedVariableSummaries-June2020.csv")
```
...
Lake_List$LakeID<-as.factor(Lake_List$LakeID)
meltLakes <- melt(Lake_List[,c(1,3:6)], id=c("LakeID", "Dreissenids"), na.rm=TRUE)
...
#Write this dataframe to a csv so the values can be easily put into a word document 
write.csv(LakeSummaries, "./output_data/StratifiedLakeSummaries.csv")
```

```{r, trophic characteristics}
#melt data frame by Dreissenids and Info_type so we can summarize all our variables of interest in one code chunk 
CSLAP3<-CSLAP[CSLAP$Info_Type == "OW", c(2, 13, 17, 32)]
...
write.csv(TrophicSummaries, "./output_data/StratifiedTrophicSummaries.csv")
...
write.csv(TrophicMean, "./output_data/StratifiedTrophicMeans.csv")
```
...
Lake_List[Lake_List$Dreissenids == 0, 5] <- rep("Uninvaded", 46)
...
#Write this dataframe to a csv so the values can be easily put into a word document 
write.csv(redVariableSummaries, "./output_data/StratifiedredVariableSummaries.csv")
```
...
#Write this dataframe to a csv so the values can be easily put into a word document 
write.csv(redLakeSummaries, "./output_data/StratifiedredLakeSummaries.csv")
```
...
### Open water TN:TP
```{r}
redTNTP<-lmer(log(TN_TP+.01) ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|Sample_Year) + (1|Sample_Month) + (1|LakeID) + (1|Sample_Year:LakeID), data=redOWCSLAP)
...
### Bottom Sample TP
```{r}
redBSTP<-lmer(log(TP+.01) ~ Dreissenids  + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|Sample_Year) + (1|Sample_Month) + (1|LakeID) + (1|Sample_Year:LakeID), data=redBSCSLAP)
...
```{r}
redChl<-lmer(log(Extracted_Chl.a+1) ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|LakeID) + (1|Sample_Year) + (1|Sample_Month) + (1|Sample_Year:LakeID), data=redOWCSLAP)
...
```{r}
redSecchi <- lmer(log(Secchi_Depth) ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|LakeID)+ (1|Sample_Year) + (1|Sample_Month) + (1|Sample_Year:LakeID), data=redOWCSLAP)
...
```{r}
redOWTC<-lmer(True_Color ~ Dreissenids  + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|LakeID) + + (1|Sample_Year:LakeID), data=redOWCSLAP)
...
```{r}
redSBChl<-lmer(log(ESF_FP_Chl.a) ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + (1|Sample_Month) + (1|LakeID) , data=redSBCSLAP)
...
```{r}
redSBmicro<-lmer(log(ESF_Microcystin) ~ Dreissenids + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag + log(TN_TP)  + (1|Sample_Year)  , data=redSBCSLAP)
...
```{r}
redOWmicro<-lmer(log(ESF_Microcystin) ~ Dreissenids + TN_TP + log(CA.SA) + log(Mean_Depth_m) + Percent_Ag  + (1|Sample_Year:LakeID), data=redOWCSLAP)
...
U.blooms$Dreissenids<-rep("Uninvaded", 576)
...
U.HTblooms$Dreissenids<-rep("Uninvaded", 288)
...
## HABS Frequency Plots 

```{r}
#change column name of HTblooms so that we can combine HTblooms with blooms
colnames(HTblooms)[3] <- "Bloom"

#combine dfs
allblooms<-rbind(HTblooms, blooms)

#remove columns where 0 is reported? 
allposblooms<-allblooms[allblooms$Freq > 0, ]
```

```{r}
ggplot(subset(allposblooms, Bloom == "Bloom"), aes(x=Sample_Year, y=Freq, fill=Dreissenids, color=Dreissenids))+
  geom_boxplot()+
  theme_minimal()+
  scale_y_continuous(breaks= c(0,2, 4, 6, 8, 10))+
  scale_fill_manual(values=c("#E7E7E7", "#FFFFFF"))+
  scale_color_manual(values=c("#000000", "#717171"))+
...
ggplot(subset(allposblooms, Bloom == "HT Bloom"), aes(x=Sample_Year, y=Freq, color=Dreissenids, fill=Dreissenids))+
  geom_boxplot()+
  theme_minimal()+
  scale_y_continuous(breaks= c(0,2, 4, 6, 8, 10))+
  scale_fill_manual(values=c("#E7E7E7", "#FFFFFF"))+
